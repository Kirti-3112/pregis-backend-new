const { jobsController } = require('../../../src/controllers');
...
describe('Job Controller', () => {
  it('Get Job Data : Controller', async () => {
    ...
    await jobsController.getJobData(getJobReq, getJobRes, authNext);
    expect(JobModel.find).toHaveBeenCalled();
  });
  it('Get Job Data for selectionFieldString : Controller', async () => {
    JobModel.find = jest.fn().mockImplementation(() => ({
      select: () => ({
        limit: () => ({
          skip: () => ({
  ...
  it('Get Job Data By Id: Controller', async () => {
    ...
    const getJobRes = {};
    const authNext = jest.fn();
    await jobsController.getJobDataById(getJobReq, getJobRes, authNext);
    expect(JobModel.findOne).toHaveBeenCalled();
  });
  it('Error if Job Data failed to load : Controller', async () => {
    JobModel.find = jest.fn().mockImplementation(() => ({
      undefined,
    }));
